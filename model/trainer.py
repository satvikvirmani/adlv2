...
class Trainer(abc.ABC):
        ...
        for epoch in range(epochs):
            for i, batch_i in enumerate(train_data):
                ...
                if self.mode == 'denoiser':
                    ...
                    loss_tot = torch.tensor(0.0, dtype=torch.float32).to(self.device)
    ...
    def _get_val(self, model, validation_data, step_i):
        ...
        for name, fn in self.metrics.items():
            summary[name] = torch.tensor(0.0, dtype=torch.float64).to(self.device)
        
        with torch.no_grad():
            for i, batch_i in enumerate(validation_data):

                # get ground-truth
                GT= batch_i['x'].to(torch.float32).to(self.device)

                # get noisy data
                y = batch_i['y'].to(torch.float32).to(self.device)
